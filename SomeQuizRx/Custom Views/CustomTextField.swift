...
class CustomTextField: UITextField {
    ...
    public func configure(backgroundColor: UIColor? = UIColor.textFieldBackground, font: UIFont? = UIFont.body, textColor: UIColor? = UIColor.white, placeholderTextColor: UIColor? = UIColor.textBodyInactive, borderStyle:UITextField.BorderStyle = .none, roundedCorners: CustomTextField.RoundedCorners = .none) {
    ...
    var roundedCorners: RoundedCorners {
        get {
            return currentRoundedCorners
        }
        set {
            if newValue != .none {
                roundCorners(radius: frame.height*roundedPercentage, masksToBounds: true, maskedCorners: newValue.mask)
                currentRoundedCorners = newValue
            }
        }
    }
    private var currentRoundedCorners: RoundedCorners = .none
